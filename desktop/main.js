...
function setupDownloadInterception() {
  session.defaultSession.on("will-download", (_event, item) => {
    ...
    const folder = path.join(app.getPath("documents"), "DEAR Management", `Slip Pay ${monthLabel}`);
...
async function goOffline() {
  ...
  try {
    ...
    dialog.showErrorBox("DEAR Management Desktop", "Gagal menjalankan server lokal: " + e.message);
  }
}
...
ipcMain.handle("arcorp:open-slip-folder", () => {
  const folder = path.join(app.getPath("documents"), "DEAR Management");
  fs.mkdirSync(folder, { recursive: true });

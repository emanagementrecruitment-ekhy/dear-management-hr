...
export async function POST(req: Request) {
  try {
    ...
    // name/outlet/vcr/total business logic either way.
    async function processRow(nameRaw: string, outletRaw: string, vcr: number, total: number, monthLabel: string, occurredAt: Date) {
      ...
      // Level is purely a display category here — the real per-voucher rate
      // always comes straight from the sheet's own total/vcr above, so a
      // fixed "MANUAL" (no per-outlet tier guessing) is correct regardless
      // of which levels this deployment actually uses.
      const level: EmployeeLevel = "MANUAL";
